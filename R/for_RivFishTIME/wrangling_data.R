#--------------------------------------------------------------------------------
# We considered a community with at least 15 species that are sampled
...
xtbl<-xq3q4%>%group_by(TimeSeriesID)%>%
  summarise(n=n_distinct(UnitAbundance))%>%ungroup()
...
for(i in 1:length(ts_ids)){
  ...
  if(minsp>=15){ # atleast 15 species should be present at a site to consider
