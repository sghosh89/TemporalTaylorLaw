...
get_inputmat_for_tailanal<-function(mtrim,resloc){
  ...
  id<-which(colnames(commonspmat)=="raresp")
  if(length(id)>0){
    commonspmat<-commonspmat[,-id]
  }
  # filter on species number
...
if(length(unique(mtrim$year))>=20){
  ...
  if(is.na(res)){
    bad_LakeID<-c(bad_LakeID,good_LakeID[i]) # not 15 sp included
  }
}else{
  bad_LakeID<-c(bad_LakeID,good_LakeID[i]) # not 20 years found
}
#--------------------------------------
i<-2
ans<-get_t1_t2_sampling(i=i)
...
good_LakeID<-setdiff(good_LakeID,bad_LakeID)
