...
#-------------Input---------------------------------
# d_allyr  :    A list of data frames, each with columns Sp and Dat
...
multcall<-function(d_allyr,resloc,nbin){
  
  lenyr<-length(d_allyr)
 
  #first initialize result receptacles for the output, here below all correlation-related
  #metric means correlations between any two rows of a species by year matrix (i.e. between any two years)
  spear<-matrix(NA,lenyr,lenyr)
  colnames(spear)<-rownames(spear)<-names(d_allyr)
 
  kend<-Corl<-Coru<-corval<-pval_BiCopIndep<-posnN<-posnI<-spear
  
  #------------------- PLOT :  copula_for_all_sp pair ----------------
  pdf(paste(resloc,"AllCops_nbin_",nbin,".pdf",sep=""),width=6*lenyr, height=6*lenyr)
  op<-par(mfrow=c(lenyr,lenyr),mar=c(3,3,3,3), mgp=c(1.5,0.5,0))
  
  for (i in c(1:lenyr)){
    for (j in c(1:lenyr)){
      
      #if(i!=j){
      ms<-vivj_matrix(d_allyr=d_allyr,i=i,j=j,level=0.05,ploton=T)
      m<-ms$mat
  ...
  res<-list(spear=spear,kend=kend,
            ...
            posnI=posnI,
            posnN=posnN,
            corval=corval # only sig. spearman correlation between any two years
            )
