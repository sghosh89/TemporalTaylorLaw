...
# sp_threshold: minimum number of species in a community (default to 15 sp)
get_input_spmat_phytoplankton<-function(Ldata,badsp,nametag,sp_threshold=15){
  ...
  if(ncol(commonspmat)>=sp_threshold){
    cat("----- nsp = ",ncol(commonspmat)," ------------\n")
    commonspmat<-commonspmat
    if(length(raresp!=0)){
...
get_input_spmat_phytoplankton(Ldata = Ldata, badsp = badsp, nametag = "L1WA",sp_threshold=15)
...
(badsp<-spfile$species[which(spfile$include==0)])
get_input_spmat_phytoplankton(Ldata = Ldata, badsp = badsp, nametag = "L2UZ",sp_threshold=15)
...
(badsp<-spfile$species[which(spfile$include==0)])
get_input_spmat_phytoplankton(Ldata = Ldata, badsp = badsp, nametag = "L3LU",sp_threshold=15)
