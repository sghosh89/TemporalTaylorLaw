...
# cc: community 
get_communitylevel_data<-function(cc,resloc,sp_threshold){
  ...
  if(ncol(commonspmat)>=sp_threshold){
    ...
    # check if any years having all zero for all species considered
    # then it's a bad year (std dev zero along row), exclude that year: it happened for did=1551, pid=2180
    allzero<-apply(commonspmat,MARGIN=1, FUN=sum)
    allzero<-unname(allzero)
    id0<-which(allzero==0)
    if(length(id0)>0){commonspmat<-commonspmat[-id0,]}
    if(nrow(commonspmat)>=20){# 20 years threshold
      commonspmat<-commonspmat
